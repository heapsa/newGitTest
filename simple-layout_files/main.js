...
IBUKI.leavePage = function(){
	if($('.one-page-enabled').length > 0 ) {
		$('header #logo, #my-menu > .mm-panel li a.external, #navigation-mobile ul li a.external').not('#my-menu > .mm-panel li a[href$="#"], #my-menu > .mm-panel li a[href^="#"], #navigation-mobile ul li a[href$="#"], #navigation-mobile ul li a[href^="#"], #my-menu > .mm-panel li a[target="_blank"], #navigation-mobile ul li a[target="_blank"]').click(function(event){
...
IBUKI.setEqualsColumnsHeight = function(){
	if( $('.row-color').length > 0 ) {
        $('.row-color').height('auto');
        var maxHeight = Math.max.apply(Math, $('.row-color').map (
            function() {
                return $(this).height();
            }
        ));
        $('.row-color').height(maxHeight);
    }
};
